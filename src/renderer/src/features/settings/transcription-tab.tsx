  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Section title="Speech">
        <Row label="Provider" htmlFor="provider" hint="Only Groq is built so far.">
          {/* §7 — the provider interface exists so this becomes a real choice
              without a rewrite. One implementation is not yet a menu. */}
          <Select id="provider" value={settings?.speechProvider ?? 'groq'} onChange={() => {}} disabled>
            <option value="groq">Groq · Whisper Large v3 Turbo</option>
          </Select>
        </Row>

        <Row label="Language" htmlFor="language" hint="Naming it is a little faster than detecting it.">
          <Select
            id="language"
      ...
      <Section title="Grammar cleanup">
        ...
        <p className="py-4 text-[13px] text-ink-muted">
          Off. Running the transcript through an LLM to fix grammar deleted words in testing — one
          word vanished from the same sentence every attempt. It returns as a setting once it can
          be trusted, and your raw transcript is always kept either way.
        </p>
      </Section>
    </div>
  )
